...
function renderMap() {
  const pins = WORLD.map((w) => `
    <g class="map-pin ${mapPick === w.id ? 'on' : ''}" data-cc="${w.id}" transform="translate(${w.x},${w.y})">
      ...
      <circle r="${mapPick === w.id ? 6.5 : 4.5}" class="pin-dot"></circle>
      <text x="${w.lx || 0}" y="${w.ly != null ? w.ly : -9}" text-anchor="middle" class="pin-l">${w.id.toUpperCase()}</text>
    </g>`).join('');
  $('worldMap').innerHTML =
    `<path d="${WORLD_LAND}" class="map-land" fill-rule="evenodd"/>` + pins;
  renderMapInfo();
}
