...
/* 성공 시: 수입 변화의 절반이 저축에 반영 (가정) — 목표가 지금보다 낮으면 음수도 정직하게 */
function chalBoost() {
  const ci = challengeInfo();
  return ci ? Math.round((ci.targetSal - S.salary) * 0.5) : 0;
}
...
/* 도전 화면에 보여줄 '성공 시 저축 변화' — 꿈의 직업은 (꿈의 저축 − 원래 저축), 창업은 수입 변화 절반 */
function chalBoostShown() {
  return S.jobPath === 'future' ? S.save - S.baseSave : chalBoost();
}
/* '지금 이대로'의 삶 갱신 — 도전 중(future)이 아닐 때의 수입·저축이 기준선이 된다 */
function syncBase() {
  if (S.jobPath !== 'future') { S.baseSalary = S.salary; S.baseSave = S.save; }
}
...
function contribAt(m, extra, noEvents) {
  ...
  if (ciA) {
    ...
    if (S.jobPath === 'future') {
      /* 꿈의 직업: 화면 수치는 꿈의 삶(S.save) — 준비 기간과 실패는 원래 삶(baseSave) */
      if (!won || m / 12 < ciA.years) c = S.baseSave + (extra || 0);
    } else if (won && m / 12 >= ciA.years) {
      c += chalBoost();   // 창업: 성공 후 수입 변화의 절반
    }
  }
...
function syncJobLine() {
  ...
  if (S.jobPath === 'future') {
    ...
    $('jobLine').innerHTML = t('job.chalLine', {
      ...
      save: money(S.save), save2: money(Math.max(0, S.save + chalBoost())),
    });
  }
}
