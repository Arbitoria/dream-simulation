...
function renderJobs() {
  ...
  $('gridJob').innerHTML = JOBS.map((j) => `
    <button class="card ${S.job === j.id ? 'on' : ''}" data-job="${j.id}" type="button">
      <span class="ce">${j.e}</span><span class="nm">${j.nm[lang] || j.nm.ko}</span>
      <span class="pr">${lang === 'ko' ? per + money(j.sal) : money(j.sal) + ' ' + per}</span>
    </button>`).join('') + `
    <button class="card ${S.job === 'custom' ? 'on' : ''}" data-job="custom" type="button">
      <span class="ce">✍️</span><span class="nm">${t('job.custom')}</span>
...
function renderEvents() {
  ...
  $('eventList').innerHTML = EVENTS.map((ev, i) => {
    ...
    const fx = (c) => {
      const parts = [];
      if (c.save) parts.push(`<span class="money-fx ${c.save < 0 ? 'neg' : ''}">${c.save > 0 ? '+' : '−'}${money(Math.abs(c.save))}/${lang === 'ko' ? '월' : 'mo'}</span>`);
...
function syncSitOutputs() {
  $('salaryOut').textContent = money(S.salary);
  $('rentOut').textContent = money(S.rent);
  $('homeValueOut').textContent = money(S.homeValue);
  $('fixedOut').textContent = money(S.fixedCost);
  $('saveOut').textContent = money(S.save);
  $('assetsOut').textContent = money(S.assets);
...
function renderGen() {
  ...
  const j = jobOf(S.job);
  $('lifeScore').innerHTML = `
    <div class="ls-col"><div class="lr-l">${t('life.scoreJob')}</div><div class="lr-v">${j ? (j.nm[lang] || j.nm.ko) : '—'}</div></div>
...
function decodeState(str) {
  ...
  S.job = p[19] === 11 ? 'custom' : (p[19] >= 1 && p[19] <= JOBS.length ? JOBS[p[19] - 1].id : null);
...
function bind() {
  document.body.addEventListener('click', (e) => {
    ...
    const jb = e.target.closest('[data-job]'); if (jb) {
      S.job = jb.dataset.job;
      const j = jobOf(S.job);
      if (j) { S.salary = j.sal; S.save = Math.round(j.sal * 0.3); }
      else if (S.job === 'custom') S.save = Math.round(S.salary * 0.3);
