...
async function share() {
  /* 공유 링크는 항상 그래프가 있는 계산 화면(9)으로 열린다 · 엔딩 칭호 포함 */
  ...
  const months = monthsToReach(N);
  const reachAge = months === Infinity ? S.age + 30 : S.age + Math.round(months / 12);
  const endTitle = t('end.' + endingOf(months, reachAge, happiness()));
  try { if (navigator.share) { await navigator.share({ title: 'Dream Simulation — ARBITORIA', text: endTitle, url: link }); return; } } catch (_) {}
  try { await navigator.clipboard.writeText(link); toast(t('toast.copied')); } catch (_) { toast(t('toast.manual')); }
}
