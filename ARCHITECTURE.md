...
```
0 언어(ko/en/fr) → 1 성별 → 2 나이 → 3 직업(10+직접입력) → 4 나의 현실(수입·저축·자산…)
→ 5 진단 → 6 드림카(10) → 7 도시(10) → 8 여행(10)
→ 9 계산(종자돈+투자 슬라이더+도달 시각화) → 10 인생 이벤트(5개 선택지) → 11 세대 곡선(엔딩+공유)
```
...
- 공유 링크는 `#d=` 해시 22필드로 전체 상태를 실어 나른다 → 받는 사람은 바로 9번 화면(그래프)에서 시작.
