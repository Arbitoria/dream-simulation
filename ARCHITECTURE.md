...
- [ ] **습관 시뮬레이터** (2026-07-28 메모) — 원칙: ① 단위는 돈이 아니라 **시간**("은퇴가 4년 빨라진다") ② 훈계 금지, "만약에" 슬라이더만 ③ 명목 아닌 **실질 수익률** 명시 ④ 훅 = 인식 vs 실제 지출 갭 (벨기에 Statbel 2022: 외식 7%·문화여가 7.7% ≫ 주류담배 1.7% — 끊어야 한다고 믿는 것과 실제로 큰 것이 다르다) ⑤ 계산 코어는 기존 엔진 재사용, 입력만 다르게. 첫 조각 '시작 시점 비교'는 계산 화면에 반영됨(#whenCard)
